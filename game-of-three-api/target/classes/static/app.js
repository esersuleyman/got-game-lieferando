...
function connect() {
    ...
    stompClient.connect({}, function (frame) {
        ...
        stompClient.subscribe('/topic/gameOfThree', function (gameStatus) {
            ...
            $("#playAutomatic").prop("disabled", isCurrentPlayer(gameStatus.lastPlayer));
        });
    });
}
...
function startGame(gameMode) {
    ...
    if(gameMode == 'HUMAN_AUTO' || gameMode == 'AI_AUTO'){
        $("#playMinusOne").hide();
        $("#playZero").hide();
        $("#playOne").hide();
    } else{
...
function play(inputNumber) {
	...
	else if((parseInt($("#currentNumber").val()) + inputNumber) % 3 != 0) {
		$("#gameStatus").append(
...
function showGameStatus(gameStatus) {
    if(gameStatus.state == 'WAITING_FOR_PLAYER') {
    	if(isCurrentPlayer(gameStatus.lastPlayer) && !(gameStatus.gameMode == 'AI' || gameStatus.gameMode == 'AI_AUTO')) {
    		$("#gameStatus").append(
...
function onChangePlayerName () {
	...
	$("#start-auto").prop("disabled", $( "#playerName").val() == '');
    $("#start-ai-auto").prop("disabled", $( "#playerName").val() == '');
}
...
$(function () {
    ...
    $( "#start-auto" ).click(function() { startGame('HUMAN_AUTO'); });
    $( "#start-ai-auto" ).click(function() { startGame('AI_AUTO'); });
    $( "#playMinusOne" ).click(function() { play(-1); });
    $( "#playZero" ).click(function() { play(0); });
    $( "#playOne" ).click(function() { play(1); });
    $( "#playAutomatic" ).click(function() { play('Auto'); });
